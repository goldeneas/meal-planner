...
export async function countPlannedMeals(db) {
    return await queryFirstAsync(db, "SELECT COUNT(*) AS count FROM Meal")
}

export async function countSavedRecipes(db) {
    return await queryFirstAsync(db, "SELECT COUNT(*) AS count FROM Recipe")
}

export async function countExpiringProducts(db) {
    return await queryFirstAsync(db, "SELECT COUNT(*) AS count FROM PantryProduct\
...
export async function countMissingFood(db) {
    return await queryFirstAsync(db, "SELECT COUNT(DISTINCT food) AS count FROM Ingredient AS I\
    WHERE\
...
export async function countAvgRecipePrepTime(db) {
    return await queryFirstAsync(db, "SELECT AVG(preparationTimeMinutes) AS count FROM Recipe")
}

export async function getMostUsedIngredients(db) {
    return await queryAllAsync(db, "SELECT F.name, COUNT(I.id) FROM Ingredient AS I\
...
export async function getMostMealsByCategory(db) {
    return await queryAllAsync(db, "SELECT RC.description, COUNT(M.id) FROM Meal AS M\
...
export async function getMostRecipesByCategory(db) {
    return await queryAllAsync(db, "SELECT RC.description, COUNT(R.id) FROM Recipe AS R\
